...
FP16 FP16::AddFP16_N_N(FP16 N1, FP16 N2) {
    ...
    if (N1.IsNull() + N2.IsNull() == 0) {
        ...
        if (flag){
            if(!((Res.man == 0) && (N1.sign != N2.sign))){
                if(N1.sign == N2.sign) Res.man++; else Res.man--;
            }
            else{
                Res.man = ((1<<manLength)-1);
                Res.exp--;
            }
        }
        else{
...
0_01110_1000011000 - need
 ...
 38.218750000000000 - A    31.421875000000000 - B
 6.80859 - my    6.79688 - need
 0_10001_1011001111 - my
 0_10001_1011001100 - need

 38.218750000000000 - A    31.453125000000000 - B
 6.77734 - my    6.76562 - need
 0_10001_1011000111 - my
 0_10001_1011000100 - need

 38.218750000000000 - A    31.484375000000000 - B
 6.74609 - my    6.73438 - need
 0_10001_1010111111 - my
 0_10001_1010111100 - need
 
 4.175781250000000 - A    3.642578125000000
 
 4.101562500000000 - A    3.853515625000000 - B
 */
...
void Test_Sub() {
    for (size_t sign1 = 0; sign1 < 1; ++sign1) {
        for (size_t exp1 = 16; exp1 < (1 << expLength); ++exp1) {
            for (size_t man1 = 0; man1 < (1 << manLength); ++man1) {
                for (size_t sign2 = 0; sign2 < 1; ++sign2) {
                    for (size_t exp2 = 16; exp2 < (1 << expLength); ++exp2) {
