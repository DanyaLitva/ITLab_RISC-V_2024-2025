...
double fp64perc =1.e-10;
float fp32perc =0.00048828125;
size_t count4pre = 4;
...
TDynamicVector<double> Gauss_Seidel_float_double(TDynamicMatrix<double> A, TDynamicVector <double> b, double ref) {
    ...
    cout<<endl<<"float ";
    auto start = std::chrono::steady_clock::now();
    x_fp32 = Gauss_Seidel_4pre(A_fp32, x_fp32, b_fp32, fp32perc);
    auto end = std::chrono::steady_clock::now();
    double elapsed = std::chrono::duration_cast<std::chrono::milliseconds > (end - start).count();
...
TDynamicVector<double> Gauss_Seidel_fp16_fp32(TDynamicMatrix<double> A, TDynamicVector <double> b, double ref) {
    ...
    cout<<"FP16 ";
    auto start = std::chrono::steady_clock::now();
    x_fp16 = Gauss_Seidel_4pre(A_fp16, x_fp16, b_fp16, FP16(0,10,0));
    auto end = std::chrono::steady_clock::now();
    double elapsed = std::chrono::duration_cast<std::chrono::milliseconds > (end - start).count();
...
int main() {
    ...
    for(size_t temp_it = 0; temp_it < count_it;temp_it++){
        ...
        b.generate();
        cout<<endl<<"Iteration №"<<temp_it+1<<endl;
