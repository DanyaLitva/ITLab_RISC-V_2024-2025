...
//		cout << hex << x << endl; //
	...
	static uint64_t zagryadskov_iter(uint64_t l, uint32_t r, uint64_t y, int iterCount = 25) { // l\r = y; r*y = l
		uint64_t tmp;
		l <<= 32; // 16
		...
		currentBit >>= 40; // 47 bits are correct // mb 48?
		for (int _ = 0; _ < iterCount; ++_) {
			//			tmp = r * y; // r, y - 64 bit
			tmp = special_mul3(r, y); // extend tmp and l up to the 64 bit long 
...
//		y = (y >> 16) + ((y & 0xFFFF) > 0x8000) + ((y & 0x1'FFFF) == 0x1'8000); // last 16 bit stored 
		if ((y & 0x7F80'0000'0000) == 0x7F80'0000'0000) return 0x7F80'0000'0000;
		return y;
	}
	...
	static uint32_t div2(uint32_t l, uint32_t r, float& example) noexcept {
		...
		//		x = bigx;
		// page 135, theorem 4.17
		
		// MY ITERATION PART
		
		uint64_t y = FP32::special_mul2(l, bigx); // y = l * x = l / r
		y = zagryadskov_iter(l, r, y); // recalculate, because y * r = l. IF DOESNT WORK, MAKE Y BIGGER (or dont)
		eres += (y >> 23 + 16) - 126; // + 16 because iter returns 16 bit longer

		uint32_t tmpy = (y >> 16) + ((y & 0xFFFF) > 0x8000) + ((y & 0x1'FFFF) == 0x1'8000);
		cout << FP32(l).example << " " << FP32(r).example << " " << FP32(tmpy).example << " " << dec << FP32(FP32(l).example / FP32(r).example).data - tmpy << hex << endl;
		cout << dec << eres << hex << endl;
//		y = uint64_t(FP32(FP32(l).example / FP32(r).example).data) << 16; // TO CHECK
		

		// BOOK PART. PAGE 132, THEOREM 4.16
		// fmaf: x * y + z
		/*
		uint32_t q = FP32::special_mul(l, bigx); // approximation of l / r
		uint32_t y = (bigx >> 16) + ((bigx & 0xFFFF) > 0x8000) + ((bigx & 0x1'FFFF) == 0x1'8000); // approximation of 1 / r
		l += 0x1ul << 23; // to diapason [1; 2)
		r += 0x1ul << 23; // to diapason [1; 2)
		
		q = FP32::usub(q);
		uint32_t df = FP32(std::fmaf( FP32(q).example, FP32(r).example, FP32(l).example )).data; // r = l - q*r
		q = FP32::usub(q);
		q = FP32(std::fmaf(FP32(y).example, FP32(df).example, FP32(q).example)).data; // q = q + y*df
		eres += (q >> 23) - 126; // and eres

		cout << FP32(l).example << " " << FP32(r).example << " " << FP32(q).example << " " << dec << FP32(FP32(l).example / FP32(r).example).data - q << hex << endl;
		cout << dec << eres << hex << endl;
		y = q;
		*/
...
//			y = zagryadskov_iter(lCopied, rCopied, y);
			...
			y = (y >> 16) + ((y & 0xFFFF) > 0x8000) + ((y & 0x1'FFFF) == 0x1'8000); // rounding
			uint32_t z = (eres << 23) + (y & 0x007F'FFFF); // making correct exponent
...
/*			y = (y & 0x007F'FFFF) + 0x0080'0000;
			uint64_t shift = 1ull << (-eres + 1); // +16 bits longer
			uint32_t z = (y >> (-eres + 1)) +
				((y & (shift - 1)) > (shift >> 1)) +
				(((y & ((shift << 1) - 1))) == (shift + (shift >> 1))); */

			y = (y & 0x007F'FFFF'FFFF) + 0x0080'0000'0000;
			uint64_t shift = 1ull << (-eres + 1 + 16); // +16 bits longer
			uint32_t z = (y >> (-eres + 1 + 16)) +
				((y & (shift - 1)) > (shift >> 1)) +
				(((y & ((shift << 1) - 1))) == (shift + (shift >> 1)));
...
public:
	...
	void run2() {
		...
		vector<uint32_t> vl = { 0x11000, 0x11000, 0x11000, 0x11000, 0x11000, 0x11000, 0x3c900000, 0x11000, 0x1980, 0x1980, 0x1980, 0x6f90e, 0xff02, 0x1000102, 0xcdb31d0 }; // {0x11000, 0x40011000, 0x811000, 0x811000, 0xaec000, 0xb85000, 0x14ffd180, 0x17ffe800, 0x2e7fd180, 0x317fe800, 0x47ffd180, 0x4affe800, 0x11000, 0x11000};
		vector<uint32_t> vr = { 0x3f00c000, 0x42719000, 0x41f10000, 0x42f11000, 0x3c801000, 0x48004000, 0x80009000, 0x3c0e6000, 0x87ff, 0x30d69c11, 0x3a4c4bdc, 0x9ea35ab1, 0xbc7fee96, 0x7f000878, 0xaa2f2fd3 }; // {0x231000, 0x80231000, 0x511d000, 0x520b000, 0x6fdc000, 0x7ecd000, 0xb47fdc3a, 0x98ffeffe, 0xb47fdc3a, 0x98ffeffe, 0xb47fdc3a, 0x98ffeffe, 0x1166000, 0x48004000 };
		for (size_t i = from; i < vl.size(); ++i) {
...
//			res = FP32::div4(uint32_t(vl[i]), uint32_t(vr[i]), f);
		...
		for (lc = 0x00000000; lc <= 0xFFFFFFFF; lc += 65282) { // 6528 69632 0x11000
			for (rc = 0x00000000; rc <= 0xFFFFFFFF; rc += 69631) {
...
//	cout << 24.0f / 17.0f << " " << hex << FP32(24.0f / 17.0f).data << endl;
//	cout << -8.0f / 17.0f << " " << hex << FP32(-8.0f / 17.0f).data << endl;
//	return 0;
